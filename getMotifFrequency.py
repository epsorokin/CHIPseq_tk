...
header= list(["Peak or Gene ID", "Gene Name", "Motif", "String Found", "Strand", "Distance from peak center"]) 
...
for fasta in fastafile:
	...
	for pos1, seq1 in amotif.instances.search(fasta.seq):
		...
		motif_entry=[fasta.id, fasta.name, str(seq1), '+', pos1- center]
	...
	for pos2, seq2 in rc.instances.search(fasta.seq):
		...
		motif_entry=[fasta.id, fasta.name, str(seq2), '-', pos2-center]
